...
import com.backend.OrderMicroservice.repository.OrderRepository;
import com.backend.OrderMicroservice.requestDTO.OrderRequestDTO;
import com.backend.OrderMicroservice.requestDTO.OrderedProductsList;
...
import com.backend.OrderMicroservice.services.OrderService;
import com.backend.PaymentMicroservice.controllers.PaymentController;
...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderResponseDTO placeOrder(OrderRequestDTO orderRequest) {
        ...
        try {
            ...
            PaymentResponse paymentResponse = this.paymentController.collectPayment(paymentRequest);
    ...
    public OrderResponseDTO getOrderDetails(String order_Id, String user_id) {
        ...
        OrderEntity order = orderRepository.findOrderByOrder_idAndUser_id(order_Id, user_id)
                .orElseThrow(() -> new CustomException("Order not found for the order Id:" + order_Id, "NOT_FOUND"));
    ...
    private List<ProductResponseDTO> fetchProductData(List<OrderedProductsList> orderedProductsList, Instant order_date) {
        List<CompletableFuture<ProductResponseDTO>> futures = orderedProductsList.stream()
                .map(orderedProduct -> CompletableFuture.supplyAsync(() -> {
                    ...
                    ProductResponseDTO response = this.productController.getParticularProduct(orderedProduct.getProduct_id()).getBody();
