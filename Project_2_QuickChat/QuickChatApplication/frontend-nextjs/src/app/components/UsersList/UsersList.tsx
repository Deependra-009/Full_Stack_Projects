...
export const UsersList = () => {
  ...
  const [groupList, setGroupList] = useState<GroupEntity[]>([]);
  ...
  const getGroupList=useSelector(fromAppSelector.getGroupList);
  ...
  const openSelectedGroupChats = (
    index: number,
    chatdata: GroupEntity
  ) => {
    console.log("=>",chatdata);

    fromAppStore.setSelectedGroup(dispatch, chatdata);
  ...
  const getSVGIcon = (item: ConversationResponse) => {
    const lastMessage =
      item?.messageResponseList[item?.messageResponseList.length - 1];

    if (
      lastMessage.messageDeliverableAt == null &&
      lastMessage.messageSeenAt == null
    )
      return "SINGLE_TICK_SVG";
    if (
      lastMessage.messageDeliverableAt != null &&
      lastMessage.messageSeenAt == null
    )
      return "DOUBLE_TICK_SVG_UNREAD";
    if (
      lastMessage.messageDeliverableAt != null &&
      lastMessage.messageSeenAt != null
    )
      return "DOUBLE_TICK_SVG_READ";
    return "";
  };
  return (
    <>
      <div className="w-[350px] h-[100vh] border-r-[2px] border-r-teal-400">
        ...
        <div className="w-[100%] px-2 py-1 h-[90vh]  overflow-quick overflow-y-scroll">
          ...
          {friendList &&
            ...
            {groupList &&
            groupList.map((item: GroupEntity, index) => (
              <div
                key={index}
                onClick={openSelectedGroupChats.bind(null, index, item)}
                className={`${
                  (item.chatID!=undefined && selectedGroup?.chatID == item.chatID)
                    ? "text-white bg-teal-500 rounded-[10px]"
                    : "text-black"
                }  py-2 cursor-pointer  my-1  px-1 w-[100%] h-[75px]  flex items-center`}
              >
                ...
                <div className="w-[50%] h-[55px] overflow-hidden flex flex-col justify-around px-1">
                  ...
                  {(item?.messageResponseList?.length == 0 || item?.messageResponseList==undefined) && (
                    <div className="w-[100%] flex items-center">
                      <div className=" text-gray-600">No message</div>
                    </div>
                  )}
